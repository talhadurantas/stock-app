...
with st.sidebar:
    ...
    with st.form(key='my_form'):
        ...
        # We make Calendar the default (index=0)
        # But give the option to switch to Text for mobile users
        input_method = st.radio(
            "Input Method:",
            ["Calendar Picker (Standard)", "Text Input (Mobile Fix)"],
            index=0
        )

        if input_method == "Calendar Picker (Standard)":
            # STANDARD CALENDAR
            ...
            # MOBILE TEXT FIX
            st.caption("Type dates as YYYY-MM-DD (e.g., 2023-01-01)")
            col1, col2 = st.columns(2)
            with col1:
                start_txt = st.text_input("Start", value="2023-01-01")
            with col2:
                end_txt = st.text_input("End", value=str(pd.to_datetime("today").date()))
            
            # Error handling for text input
            try:
                start_date = pd.to_datetime(start_txt)
                end_date = pd.to_datetime(end_txt)
            except:
